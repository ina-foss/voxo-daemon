...
while (<PIPE>) {
	    		...
	    		if(scalar(@tab) > 0){
					...
					$wordSel =~ s/\&/_et_/g;	
			    		...
			    		if( ($wordSel ne "eps") && ($wordSel ne "euh") && ($wordSel !~ /\</) && ($wordSel !~ /\[/) ){
			    			$sentence .= "<word sel=\"$wordSel\" start=\"".($startSeg+$tpsDeb)."\" length=\"".($tpsFin-$tpsDeb)."\" scoreConfiance=\"$scoreSel\">\n";
				    		foreach(@tab){
				    			my $cle = $_;
				    			if($cle =~ /([^\_]+)\_(\S+)/){
								my $prop=$2;
    ...
    if(length($sentence)>0){
    	...
    	if($monLocuteurCluster eq "-1"){
	    	print "<sentence start=\"".$seg[1]."\" end=\"".$seg[2]."\" locuteur=\"".$seg[4]."\" type=\"$type\" sexe=\"$sexe\">\n";
	    	print $sentence;
	    	print "</sentence>\n";
    	}else{
    		...
    		print "<sentence start=\"".$seg[1]."\" end=\"".$seg[2]."\" locuteur=\"".$monLocuteurCluster."\" type=\"$type\" sexe=\"$sexe\">\n";
	    	print $sentence;
	    	print "</sentence>\n";
	    }
    }    
